...
*   **🌌 星际穿越 (原速激星空):** 引入了动态原点漂移（Lissajous 轨迹）算法，并将最大飞行速度优化为原来的 50%，创造出更优雅的太空巡航感。
*   **🫧 微观液泡:** 新增**景深 (Depth of Field)** 模拟。现在的泡泡会根据其虚拟深度呈现出不同的清晰度、散景 (Bokeh) 高光以及边缘光照效果。

### ✨ v0.6.7 修复补丁：部署问题

*   **🔧 构建配置修复:** 将 Vite 构建输出目录从 `dist` 更改为 `build`，以严格匹配项目文档及 Vercel/Netlify 等平台的默认或推荐配置。
