  ...
  const ControlPanelButton = ({ onClick, label, active, hintKey }: any) => {
    ...
    return (
      <div className="relative flex-1" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        ...
        <button 
          onClick={onClick} 
          className={`w-full py-4 rounded-xl border text-[10px] font-black uppercase tracking-[0.15em] transition-all duration-300 ${active ? 'bg-white/15 border-white/30 text-white shadow-[inset_0_2px_10px_rgba(255,255,255,0.05)]' : 'bg-white/[0.02] border-white/5 text-white/40 hover:text-white hover:bg-white/5 hover:border-white/10'}`}
        >
          {label}
        </button>
      </div>
          ...
          <div className="max-h-[85vh] overflow-y-auto custom-scrollbar pt-10 pb-32 px-10">
            <div className="max-w-6xl mx-auto space-y-12">
              <div className="flex flex-col md:flex-row justify-between items-center gap-8 border-b border-white/10 pb-10">
                <div className="flex bg-white/[0.04] p-2 rounded-2xl border border-white/10 overflow-x-auto max-w-full scrollbar-hide gap-1">
                  {(['visual', 'audio', 'ai', 'system'] as TabType[]).map(tab => (
                    <button 
                      key={tab} 
                      onClick={() => setActiveTab(tab)} 
                      className={`px-6 py-3 rounded-xl text-[11px] font-black uppercase tracking-[0.2em] transition-all duration-300 flex-shrink-0 ${activeTab === tab ? 'bg-white/20 text-white shadow-[0_8px_25px_rgba(255,255,255,0.08)]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}
                    >
                      {t.tabs[tab]}
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 items-stretch">
                {activeTab === 'visual' && (
                  <>
                    <div className="flex flex-col bg-white/[0.03] border border-white/10 rounded-[2rem] p-8 h-full space-y-6 shadow-2xl">
                    ...
                    <div className="flex flex-col bg-white/[0.03] border border-white/10 rounded-[2rem] p-8 h-full space-y-8 shadow-2xl">
                      ...
                      <div className="space-y-3 pt-6 border-t border-white/10 flex-1 flex flex-col justify-end">
                          <div className="flex items-center justify-between p-4 bg-white/[0.02] rounded-2xl border border-white/5 relative group">
                             <span className="text-[11px] font-black uppercase text-white/60 tracking-widest">{t.autoRotate}</span>
                             <button onClick={() => setSettings({...settings, autoRotate: !settings.autoRotate})} className={`w-12 h-6.5 rounded-full relative transition-all duration-500 ${settings.autoRotate ? 'bg-blue-600' : 'bg-white/10'}`}>
                          ...
                          <button onClick={resetVisualSettings} className="w-full py-4 bg-white/[0.02] border border-white/5 rounded-xl text-[10px] font-black uppercase tracking-widest text-white/50 hover:text-white hover:bg-white/[0.06] hover:border-white/20 transition-all duration-300 flex items-center justify-center gap-3">
                ...
                {activeTab === 'audio' && (
                  <>
                    <div className="bg-white/[0.03] border border-white/10 rounded-[2rem] p-8 space-y-8 shadow-2xl">
                    ...
                    <div className="bg-white/[0.03] border border-white/10 rounded-[2rem] p-8 space-y-10 shadow-2xl">
                    ...
                    <div className="bg-white/[0.03] border border-white/10 rounded-[2rem] p-8 space-y-6 shadow-2xl">
                      <span className="text-[11px] font-black uppercase text-white/50 tracking-[0.25em] block ml-1">{t.fftSize}</span>
                      <div className="grid grid-cols-2 gap-3">
                         {[512, 1024, 2048, 4096].map(size => (
                           <button key={size} onClick={() => setSettings({...settings, fftSize: size})} className={`py-4 rounded-xl border text-[13px] font-mono font-bold transition-all duration-300 ${settings.fftSize === size ? 'bg-white/20 border-white/40 text-white shadow-[inset_0_2px_10px_rgba(255,255,255,0.05)]' : 'bg-white/[0.02] border-white/5 text-white/40 hover:text-white hover:border-white/20'}`}>
                             {size}
                           </button>
                         ))}
                      </div>
                ...
                {activeTab === 'system' && (
                  <>
                    <div className="bg-white/[0.03] border border-white/10 rounded-[2rem] p-8 space-y-8 shadow-2xl">
                      ...
                      <button onClick={resetSettings} className="w-full py-4.5 bg-red-500/10 border border-red-500/30 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] text-red-400 hover:bg-red-500/20 hover:text-red-300 transition-all duration-300" title={t.reset}>
                        {t.reset}
                      </button>
                    </div>
                    <div className="bg-white/[0.03] border border-white/10 rounded-[2rem] p-8 space-y-6 md:col-span-2 shadow-2xl">
                      <span className="text-[11px] font-black uppercase text-white/50 tracking-[0.25em] block ml-1">{t.appInfo}</span>
                      <div className="bg-white/[0.02] border border-white/5 p-8 rounded-3xl space-y-8 shadow-inner">
