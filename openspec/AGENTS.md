...
2. 选择唯一的动词开头 `change-id` 并搭建 `proposal.md`、`tasks.md`、可选的 `design.md` 和 `openspec/changes/<id>/` 下的规范增量。
...
- ADDED：引入可以作为独立需求的新功能或子功能。当变更是正交的时（例如，添加 "Slash Command Configuration"），而不是改变现有需求的语义时，首选 ADDED。
- MODIFIED：更改现有需求的行为、范围或验收标准。始终粘贴完整的、更新后的需求内容（标题 + 所有场景）。归档器将用您在此处提供的内容替换整个需求；部分增量会丢失先前的详细信息。
- RENAMED：仅在名称变更时使用。如果您还更改了行为，请使用 RENAMED（名称）加上 MODIFIED（内容）引用新名称。
...
4) 确保标题文本完全匹配（不区分空白）并至少保留一个 `#### Scenario:`。
...
- 首选动词开头前缀：`add-`、`update-`、`remove-`、`refactor-`
